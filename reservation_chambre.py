...
class ReservationForm(Gtk.Window):
    ...
    def __init__(self):
        ...
        # Champ de saisie pour le prenom
        label_prenom = Gtk.Label(label="prenom")
        grid.attach(label_prenom, 0, 1, 1, 1)
        entry_prenom = Gtk.Entry()
        entry_prenom.set_placeholder_text("prenom")
        grid.attach(entry_prenom, 1, 1, 1, 1)

        # Champ de saisie pour le numéro de Telephone
        label_telephone = Gtk.Label(label="Telephone")
        grid.attach(label_telephone, 0, 2, 1, 1)
        entry_telephone = Gtk.Entry()
        entry_telephone.set_placeholder_text("Numéro de Telephone")
